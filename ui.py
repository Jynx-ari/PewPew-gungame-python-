...
def draw_hud(screen, font, score, hp, energy, current_weapon, weapon_names, explosion_ammo, spread_ammo, game_timer, current_stage, shield_is_disabled, blink=False):
    ...
    pygame.draw.rect(screen, (20, 20, 40), (20, 60, 150, 15))
    if not shield_is_disabled and energy > 10:
        energy_color = (0, 255, 255) # Cyan
    ...
    # Score
    score_color = (0, 255, 0) # Neon Green
    if (score % 100 < 10) and (score >= 100):
        current_time = pygame.time.get_ticks()
        flash_interval = 300
        score_color = (255, 255, 0) if ((current_time // flash_interval) % 2) == 0 else (255, 100, 0)
    screen.blit(font.render(f"SCORE: {score}", True, score_color), (20, 85))
    
    # HP
    screen.blit(font.render(f"HP: {hp}", True, (255, 50, 50)), (20, 110))
    
    # Weapon
    weapon_text = f"WEAPON: {weapon_names[current_weapon]}"
    weapon_color = (255, 0, 255) # Magenta
    if score < 200:
...
def draw_menu(screen, font):
    ...
    title_main = font.render(title_text, True, (0, 255, 255)) # Cyan
...
def draw_difficulty_menu(screen, font, current_index, is_hardcore=False):
    ...
    # Header
    head_surf = font.render(title, True, (0, 255, 255)) # Cyan
    ...
    for i, item in enumerate(items):
        ...
        if selected:
            ...
            pygame.draw.polygon(screen, (0, 255, 0), bg_points) # Neon Green
            text_color = (0, 0, 0) # Contrast black
            # Selection shake
...
def draw_pause_menu(screen, font, current_index, options):
    ...
    pygame.draw.rect(screen, (0, 255, 255), menu_rect, 3) # Cyan border
    ...
    for i, option in enumerate(options):
        color = (0, 255, 255) if i == current_index else (200, 200, 200)
